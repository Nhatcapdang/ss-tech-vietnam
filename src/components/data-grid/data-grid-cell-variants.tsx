...
import { Status, StatusLabel } from '../ui/status'
import {
  TagsInput,
  TagsInputInput,
  TagsInputItem,
...
export function TagsInputCell<TData>({
  cell,
  tableMeta,
  rowIndex,
  columnId,
  ...
  // Sync when row data changes externally
  useIsomorphicLayoutEffect(() => {
    setValues((cell.getValue() as string[]) ?? [])
  }, [cell.getValue()])
  ...
  const placeholder =
    cellOpts?.variant === 'tags-input'
      ? (cellOpts.placeholder ?? 'Add email...')
      : 'Add email...'

  const onValidate =
    cellOpts?.variant === 'tags-input' ? cellOpts.onValidate : undefined

  // same sideOffset calculation as MultiSelectCell
  const sideOffset =
    rowHeight === 'short' ? -33 : rowHeight === 'medium' ? -41 : -49
      ...
      {isEditing && (
        <Popover open={isEditing} onOpenChange={onOpenChange}>
          ...
          <PopoverContent
            ...
            align="start"
            sideOffset={sideOffset}
            className="w-[400px] rounded-none p-2"
            ...
            >
              <TagsInputList>
                {values.map((email, index) => (
                  <TagsInputItem key={email + index.toString()} value={email}>
                    {email}
                  </TagsInputItem>
                ))}
                <TagsInputInput placeholder={placeholder} autoFocus />
              </TagsInputList>
            </TagsInput>
          </PopoverContent>
        </Popover>
      )}
      {values.length > 0 && values.some(email => email.length > 0) && (
        <BadgeOverflow
          items={values}
