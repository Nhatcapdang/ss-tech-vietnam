...
function SpeedDialTrigger(props: React.ComponentProps<typeof Button>) {
  ...
  const onClick = React.useCallback(
    (event: React.MouseEvent<TriggerElement>) => {
      ...
      } as BaseUIEvent<React.MouseEvent<TriggerElement>>)
      if (event.defaultPrevented) return
...
const speedDialItemVariants = cva(
  'flex items-center gap-2 transition-all [transition-delay:var(--speed-dial-delay)] [transition-duration:var(--speed-dial-animation-duration)] data-[state=closed]:opacity-0 data-[state=open]:translate-x-0 data-[state=open]:translate-y-0 data-[state=open]:opacity-100 rtl:data-[state=open]:-translate-x-0',
  {
    variants: {
      side: {
        top: 'justify-end',
  ...
  const onClick = React.useCallback(
    (event: React.MouseEvent<ActionElement>) => {
      propsRef.current?.onClick?.({
        preventBaseUIHandler: () => {},
        baseUIHandlerPrevented: false,
      } as BaseUIEvent<React.MouseEvent<ActionElement>>)
      if (event.defaultPrevented) return
