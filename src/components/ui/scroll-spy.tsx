 ...
 * <ScrollSpy>
 *   <ScrollSpyNav>
 *     <ScrollSpyLink value="home">Home</ScrollSpyLink>
 *     <ScrollSpyLink value="about">About</ScrollSpyLink>
 *     <ScrollSpyLink value="contact">Contact</ScrollSpyLink>
 *   </ScrollSpyNav>
 *   <ScrollSpyViewport>
 *     <ScrollSpySection value="home">Home</ScrollSpySection>
 *     <ScrollSpySection value="about">About</ScrollSpySection>
 *     <ScrollSpySection value="contact">Contact</ScrollSpySection>
 *   </ScrollSpyViewport>
 * </ScrollSpy>
 */
...
function ScrollSpyNav(props: ScrollSpyNavProps) {
  const { asChild, className, children, ...navProps } = props

  const { dir, orientation } = useScrollSpyContext(NAV_NAME)
  const activeValue = useStore(state => state.value)

  const NavPrimitive = asChild ? SlotPrimitive.Slot : 'nav'
  const flexDirection = orientation === 'horizontal' ? 'flex-col' : 'flex-row'
  ...
  return (
    ...
    >
      <Highlight
        ...
        hover={false}
        click={false}
        className="-z-1! rounded bg-accent"
        containerClassName={cn('flex gap-2', flexDirection)}
      >
        {children}
      </Highlight>
    </NavPrimitive>
...
function ScrollSpyLink(props: ScrollSpyLinkProps) {
  ...
  return (
    <HighlightItem asChild value={linkValue}>
      <LinkPrimitive
        data-orientation={orientation}
        ...
        className={cn(
          'rounded px-3 py-1.5 text-sm font-medium text-muted-foreground transition-colors hover:text-accent-foreground data-[state=active]:text-foreground',
          className
        )}
        onClick={onLinkClick}
      />
