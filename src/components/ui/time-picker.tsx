    ...
    <TimePickerInputGroupContext.Provider value={inputGroupContextValue}>
      <PopoverAnchor asChild>
        <InputGroupPrimitive
          ...
          {...inputGroupProps}
          className={cn(
            'flex h-8 w-full cursor-text items-center gap-0.5 rounded-md border border-input bg-input/30 px-3 py-2 shadow-xs transition-shadow outline-none',
          ...
          style={
            {
              ...
              '--time-picker-minute-input-width': `${segmentPlaceholder.minute.length + 1}ch`,
              '--time-picker-second-input-width': `${segmentPlaceholder.second.length}ch`,
              '--time-picker-period-input-width': `${Math.max(segmentPlaceholder.period.length, 2) + 1}ch`,
              ...style,
...
function TimePickerTrigger(props: ButtonProps) {
  ...
  return (
    <PopoverTrigger
      ...
      {...triggerProps}
      className={cn(
        "ms-auto flex items-center text-muted-foreground transition-colors hover:text-foreground disabled:pointer-events-none [&>svg:not([class*='size-'])]:size-4",
        className
      )}
    >
      {children ?? <Clock />}
  ...
  return (
    <TimePickerGroupContext.Provider value={groupContextValue}>
      <PopoverContent
        ...
        {...contentProps}
        className={cn(
          'flex w-auto max-w-(--radix-popover-trigger-width) flex-row p-0',
          className
        )}
...
function TimePickerColumnItem(props: TimePickerColumnItemProps) {
  ...
  const onKeyDown = React.useCallback(
    ...
    [itemProps, groupContext, columnContext, value]
  )
